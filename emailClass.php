...
$unsubscribe = 'https://afrihost.sasscal.org/rainapp/unsubscribe.php?acc='.$id;
$link = 'https://afrihost.sasscal.org/rainapp/confirm.php?token='.@$hashkey.'&id='.@$id;
           /*
    $mail->Host = $settings->settings['o_host'];  // Specify main and backup SMTP servers
    $mail->SMTPAuth = true;                               // Enable SMTP authentication
    $mail->Username = $settings->settings['o_username'];                 // SMTP username
    $mail->Password = $settings->settings['o_password'];                           // SMTP password
    $mail->SMTPSecure = 'tls';                            // Enable TLS encryption, `ssl` also accepted
    $mail->Port = 587;                                    // TCP port to connect to
    */
        //create token using email, recieve the location id, save id in token
        //send email with token and id as part of link
        //click link redirects to html/js page that checks client the user is accessing page on.

    $mail->isSMTP();
    $mail->Host = $settings->settings['g_host'];  // Specify main and backup SMTP servers
    $mail->SMTPAuth = true;                               // Enable SMTP authentication
    $mail->Username = $settings->settings['g_username'];                 // SMTP username
    $mail->Password = $settings->settings['g_password'];                           // SMTP password
    $mail->SMTPSecure = 'tls';                            // Enable TLS encryption, `ssl` also accepted
    $mail->Port = 587;

    $mail->setFrom('oadc@sasscal.org', 'SASSCAL Open Access Data Centre');
    $mail->addAddress($user);  // Name is optional
    $mail->addReplyTo('oadc@sasscal.org', 'SASSCAL Open Access Data Centre');
    $mail->isHTML(true);                                  // Set email format to HTML
    $mail->AddEmbeddedImage('img/sasscal_small.png', 'logo');
    $mail->Subject = 'SASSCAL Crowdweather Welcome email';
                    //header
 $mail->Body  = '<html>
<head>
              ...
              <table>
                <tr>
                  <td>
                    ...
                    <p >
                    <center><a href="'.$link.'" target="_blank" >Please click here to confirm your email address</a></center>
                    </p>
                    ...
                    There are a few considerations that we would like to share with you:<br>
